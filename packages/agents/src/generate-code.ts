...
export interface GeneratedCode {
  ...
  envExample?: string;
  /**
   * True when workflowUsed names a real workflow, but the endpoint this
   * script actually calls isn't mentioned in any of that workflow's steps --
   * v1 only generates real code for GET endpoints, so a write-only (or
   * otherwise unmatched) workflow silently falls back to "the first GET
   * endpoint overall," which can have nothing to do with the workflow
   * description sitting right above the code. False when there's no
   * workflow targeted, or its steps do mention the endpoint used.
   */
...
function stub(lang: GenerateLang, workflowUsed: string | null, stubReason: string): GeneratedCode {
  ...
  return { code, isStub: true, stubReason, workflowUsed, syntaxValidated: false, workflowMismatch: false };
}
...
/**
 * A plain substring check on the method (e.g. "get") false-positives badly:
 * "get" is literally embedded inside "widgets", "budgets", "gadgets",
 * "targets", so a step about POSTing to /widgets would wrongly read as
 * "mentions a GET call" on any resource name that happens to contain the
 * method name as a substring. Word-boundary matching avoids that.
 */
export function stepMentionsEndpoint(step: string, endpoint: Endpoint): boolean {
  const lower = step.toLowerCase();
  const methodPattern = new RegExp(`\\b${endpoint.method.toLowerCase()}\\b`);
  return methodPattern.test(lower) && lower.includes(endpoint.path.toLowerCase());
}
...
export function selectReadEndpoint(endpoints: Endpoint[], workflow?: { steps: string[] }): Endpoint | undefined {
  ...
  const fieldComment =
    fields.length > 0
      ? `// From blueprint: ${safeBlueprintSection} -- expected response fields\n// ${fields
          ...
          .join(", ")}\n\n`
      : "";
  const mismatchComment = workflowMismatch
    ? `// Note: the "${safeBlueprintSection}" workflow's steps don't mention this call. Scout only\n// generates real code for GET endpoints in v1, so this demonstrates the auth handshake\n// and a basic read call (${endpoint.method} ${sanitizeForComment(endpoint.path)}) instead of that workflow -- adapt it for\n// the actual request the workflow describes.\n`
    : "";
...
${fieldComment}${mismatchComment}// From blueprint: ${safeBlueprintSection}
async function main() {
...
export async function generateCode(
  ...
  const workflowMismatch = workflow !== undefined && !workflow.steps.some((step) => stepMentionsEndpoint(step, endpoint));

  const code =
    lang === "ts"
      ? renderTypeScript(platform, endpoint, urlPlan, fields, blueprintSection, workflowMismatch)
      : renderPython(platform, endpoint, urlPlan, fields, blueprintSection, workflowMismatch);

  const { validated, note } = await validateSyntax(code, lang);
  const envExample = envExampleFor(platform);
  return note === undefined
    ? { code, isStub: false, workflowUsed, syntaxValidated: validated, envExample, workflowMismatch }
    : { code, isStub: false, workflowUsed, syntaxValidated: validated, syntaxValidationNote: note, envExample, workflowMismatch };
}
