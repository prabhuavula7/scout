...
export default function ChatPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  const { data: messages } = useQuery({
    queryKey: ["chat", platformId],
    queryFn: async () => (await getApi()).listChatMessages(platformId!),
    enabled: !!platformId,
  });

  const sendMessage = useMutation({
    mutationFn: async (message: string) =>
      (await getApi()).sendChatMessage(
        platformId!,
        message,
        (messages ?? []).map((m) => ({ role: m.role, content: m.content })),
      ),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["chat", platformId] });
