...
- Folding a chat thread into a handoff brief distills only what that conversation actually confirmed, no restating generic platform info, no inventing a decision that wasn't made; an empty or purely exploratory thread yields no summary section at all rather than a padded-out one.
